import React from 'react';

const Flag = () => {
	const params = {
		slidesPerView: 5,
		loop: true,
		speed: 1000,
		spaceBetween: 30,
		autoplay: {
			delay: 3000,
			disableOnInteraction: false
		},

		// Responsive breakpoints
		breakpoints: {
			1024: {
				slidesPerView: 5
			},
			768: {
				slidesPerView: 4
			},
			640: {
				slidesPerView: 3
			},
			320: {
				slidesPerView: 2
			}
		}
	};

	return (
		<section className="brand-two p-0 border-top-0">
			<div className="container">
				<div className="brand-one__carousel">
					<div className="d-flex flex-column flex-md-row justify-content-center align-items-center">
						<div className="d-flex flex-column text-center mb-4 mb-md-0 mr-0 mr-md-4">
							<a href="#">
								<img src="/assets/images/my.png" className="rounded-circle" alt="Du học Mỹ" />
							</a>
							<span className="font-weight-bold">
								<a href="#">Du học Mỹ</a>
							</span>
						</div>
						<div className="d-flex flex-column text-center mb-4 mb-md-0 mr-0 mr-md-4">
							<a href="#">
								<img src="/assets/images/canada.gif" className="rounded-circle" alt="Du học Canada" />
							</a>
							<span className="font-weight-bold">
								<a href="#">Du học Canada</a>
							</span>
						</div>
						<div className="d-flex flex-column text-center mb-4 mb-md-0 mr-0 mr-md-4">
							<a href="">
								<img src="/assets/images/uc.gif" className="rounded-circle" alt="Du học Úc" />
							</a>
							<span className="font-weight-bold">
								<a href="#">Du học Úc</a>
							</span>
						</div>
						<div className="d-flex flex-column text-center mb-4 mb-md-0 mr-0 mr-md-4">
							<a href="#">
								<img src="/assets/images/anh.png" className="rounded-circle" alt="Du học Anh" />
							</a>
							<span className="font-weight-bold">
								<a href="#">Du học Anh</a>
							</span>
						</div>
						<div className="d-flex flex-column text-center mb-4 mb-md-0 mr-0 mr-md-4">
							<a href="#">
								<img src="/assets/images/newzealand.gif" className="rounded-circle" alt="Du học New Zealand" />
							</a>
							<span className="font-weight-bold">
								<a href="#">Du học New Zealand</a>
							</span>
						</div>
					</div>
					{/* <div className="gallery">
						<a
							href="https://newgoldenroad.com/chuyen-muc/
						du-hoc-cac-nuoc/du-hoc-my/cac-truong-dai-hoc-cao-dang-my/"
						>
							<img src="assets/images/my.png" alt="Du học Mỹ" />
						</a>
						<div class="desc">
							<a
								href="https://newgoldenroad.com/chuyen-muc/
						du-hoc-cac-nuoc/du-hoc-my/cac-truong-dai-hoc-cao-dang-my/"
							>
								Du học Mỹ
							</a>
						</div>
					</div>

					<div className="gallery">
						<a
							href="https://newgoldenroad.com/chuyen-muc/
							du-hoc-cac-nuoc/du-hoc-canada/cac-truong-dai-hoc-cao-dang-canada/"
						>
							<img src="assets/images/canada.gif" alt="Du học Canada" />
						</a>
						<div class="desc">
							<a
								href="https://newgoldenroad.com/chuyen-muc/
						du-hoc-cac-nuoc/du-hoc-canada/cac-truong-dai-hoc-cao-dang-canada/"
							>
								Du học Canada
							</a>
						</div>
					</div>

					<div className="gallery">
						<a
							href="https://newgoldenroad.com/chuyen-muc/
							du-hoc-cac-nuoc/du-hoc-uc/cac-truong-dai-hoc-cao-dang-uc/"
						>
							<img src="assets/images/uc.gif" alt="Du học Úc" />
						</a>
						<div class="desc">
							<a
								href="https://newgoldenroad.com/chuyen-muc/
						du-hoc-cac-nuoc/du-hoc-uc/cac-truong-dai-hoc-cao-dang-uc/"
							>
								Du học Úc
							</a>
						</div>
					</div>

					<div className="gallery">
						<a
							href="https://newgoldenroad.com/chuyen-muc/
							du-hoc-cac-nuoc/du-hoc-anh/cac-truong-dai-hoc-cao-dang-anh/"
						>
							<img src="assets/images/anh.png" alt="Du học Anh" />
						</a>
						<div class="desc">
							<a
								href="https://newgoldenroad.com/chuyen-muc/
							du-hoc-cac-nuoc/du-hoc-anh/cac-truong-dai-hoc-cao-dang-anh/"
							>
								Du học Anh
							</a>
						</div>
					</div>

					<div className="gallery">
						<a
							href="https://newgoldenroad.com/chuyen-muc/
						du-hoc-cac-nuoc/du-hoc-new-zealand/cac-truong-dai-hoc-cao-dang-
						new-zealand/"
						>
							<img src="assets/images/newzealand.gif" alt="Du học New Zealand" />
						</a>
						<div class="desc">
							<a
								href="https://newgoldenroad.com/chuyen-muc/
						du-hoc-cac-nuoc/du-hoc-new-zealand/cac-truong-dai-hoc-cao-dang-
						new-zealand/"
							>
								Du học New Zealand
							</a>
						</div>
					</div> */}
				</div>
			</div>
		</section>
	);
};

export default Flag;
